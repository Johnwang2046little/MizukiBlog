...
export interface TimelineItem {
	...
	title: string;
	description: string;
	type: "education" | "work" | "project" | "achievement" | "play" | "event" ;
	startDate: string;
...
export const getTimelineStats = () => {
	...
	const byType = {
		...
		achievement: timelineData.filter((item) => item.type === "achievement")
			.length,
        play: timelineData.filter((item) => item.type === "play").length,
        event: timelineData.filter((item) => item.type === "event").length,
	};
